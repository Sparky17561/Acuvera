            ...
            <div className="flex flex-1 flex-wrap items-center gap-4">
              ...
              <span
                className={`font-mono text-sm font-bold ${
                  isSlaRisk ? "text-rose-600" : "text-amber-600"
                }`}
              >
                ⏱ {timeStr}
        ...
        <div className="mb-4 flex flex-col gap-1">
          {REASONS.map((r) => (
            <button
              key={r}
              onClick={() => setReason(r)}
              className={`rounded-lg px-4 py-2 text-left text-sm font-medium capitalize transition-all ${
                reason === r
...
function MyCasesPage() {
  ...
  const load = useCallback(async () => {
    try {
      const mine = await EncounterAPI.list({
        status: "",
      ...
      <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          <h1 className="flex items-center gap-3 text-3xl font-black tracking-tight text-gray-900">
            <LayoutDashboard className="h-8 w-8 text-blue-600" /> My Cases
          </h1>
        ...
        >
          <RefreshCw size={16} className="text-blue-500" /> Refresh List
        </button>
      </div>
      ...
      {cases.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-gray-200 bg-gray-50/50 py-20 backdrop-blur-sm">
          ...
          <p className="font-medium text-gray-500">
            No active cases — check <strong>Assignments</strong> for new cases
            to accept
          </p>
          <button
            onClick={load}
        ...
        <div className="grid grid-cols-1 gap-6">
          {cases.map((enc) => (
            <div
              key={enc.id}
              className={`group relative rounded-3xl border border-white/50 bg-white/70 p-6 shadow-xl backdrop-blur-xl transition-all hover:border-blue-200 hover:bg-white/80 ${
      ...
      {elements.map((el, i) => {
        if (el.type === "header") {
          return (
            <div
              key={i}
              className={`mt-2 border-b border-gray-200 pb-1 text-xs font-black uppercase tracking-wider ${
                el.isMain ? "text-gray-700" : "text-gray-500"
              }`}
            >
              {el.text}
            </div>
