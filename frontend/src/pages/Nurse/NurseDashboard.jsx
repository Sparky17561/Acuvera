        ...
        >
            ...
            >
                {advice.actions.map((action, i) => (
                    <div
                        key={i}
                        style={{ fontSize: "0.8rem", color: "#cbd5e1", lineHeight: 1.5 }}
                    >
                        {action}
                    </div>
                ))}
                    ...
                    <div>
                        ...
                        >
                            ...
                            {result.risk_prediction && (
                                <div style={{ borderBottom: "1px solid rgba(51,65,85,0.5)", paddingBottom: "0.5rem" }}>
                                    <div
                                        style={{
                                            padding: "0.5rem 1rem",
                                    ...
                                    >
                                        ...
                                        {result.risk_prediction.prediction_generated_at && (
                                            <span style={{ fontSize: "0.6rem", fontWeight: 500, textTransform: "none", color: "#94a3b8" }}>
                                    ...
                                    <div style={{ padding: "0.5rem 1rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                                        ...
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px dashed rgba(255,255,255,0.1)", paddingBottom: "0.5rem" }}>
                                            <span style={{ fontSize: "0.85rem", color: "#e2e8f0", fontWeight: 600 }}>Overall Deterioration Risk</span>
                                        ...
                                        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "0.5rem" }}>
                                            <div style={{ background: "rgba(255,255,255,0.03)", padding: "0.5rem", borderRadius: 8, display: "flex", flexDirection: "column", alignItems: "center" }}>
                                                <span style={{ fontSize: "0.6rem", color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em", fontWeight: 700 }}>ICU Admit</span>
                                                <span style={{ fontSize: "0.95rem", fontWeight: 700, color: "#cbd5e1", marginTop: "0.2rem" }}>{result.risk_prediction.icu_probability}%</span>
                                            </div>
                                            <div style={{ background: "rgba(255,255,255,0.03)", padding: "0.5rem", borderRadius: 8, display: "flex", flexDirection: "column", alignItems: "center" }}>
                                                <span style={{ fontSize: "0.6rem", color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em", fontWeight: 700 }}>Resp Failure</span>
                                                <span style={{ fontSize: "0.95rem", fontWeight: 700, color: "#cbd5e1", marginTop: "0.2rem" }}>{result.risk_prediction.respiratory_failure_risk}%</span>
                                            </div>
                                            <div style={{ background: "rgba(255,255,255,0.03)", padding: "0.5rem", borderRadius: 8, display: "flex", flexDirection: "column", alignItems: "center" }}>
                                                <span style={{ fontSize: "0.6rem", color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em", fontWeight: 700 }}>Cardiac Event</span>
                                                <span style={{ fontSize: "0.95rem", fontWeight: 700, color: "#cbd5e1", marginTop: "0.2rem" }}>{result.risk_prediction.cardiac_event_risk}%</span>
                                            </div>
                                        </div>

                                        {/* Top Drivers */}
                                        {result.risk_prediction.top_risk_drivers?.length > 0 && (
                                            <div style={{ marginTop: "0.2rem", paddingTop: "0.6rem", borderTop: "1px dashed rgba(255,255,255,0.1)" }}>
                                                <div style={{ fontSize: "0.62rem", color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em", fontWeight: 700, marginBottom: "0.4rem" }}>Top Risk Drivers</div>
                                                <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                                                    {result.risk_prediction.top_risk_drivers.map((drv, idx) => (
                                                        <div key={idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "0.75rem" }}>
                                                            <span style={{ color: "#cbd5e1" }}>{drv.label}</span>
        ...
        >
            ...
            <div style={{ flex: 1, minWidth: 120 }}>
                ...
                <div style={{ fontSize: "0.75rem", color: "#94a3b8", marginTop: 2 }}>
                    ...
                    {enc.patient_detail?.chief_complaint
                        ? ` · ${enc.patient_detail.chief_complaint}`
                        : ""}
                </div>
...
function QueuePage() {
    ...
    const escalatedEncounters = activeQueue.filter(
        (e) => e.status === "escalated",
    );
    const rejectedEncounters = activeQueue.filter(
        (e) => e.rejection_count > 0 && !e.assigned_doctor_detail,
    );
    ...
    const STATUS_FILTERS = [
        { value: "all", label: "All", count: activeQueue.length },
        {
            value: "waiting",
            label: "Waiting",
            count: activeQueue.filter((e) => displayStatus(e) === "waiting").length,
        },
        {
            value: "assigned",
            label: "Assigned",
            count: activeQueue.filter((e) => e.status === "assigned").length,
        },
        {
            value: "in_progress",
            label: "In Progress",
            count: activeQueue.filter((e) => displayStatus(e) === "in_progress")
                .length,
        },
        {
            value: "escalated",
            label: "Escalated",
            count: activeQueue.filter((e) => e.status === "escalated").length,
        },
        {
            value: "completed",
            label: "Completed",
            count: activeQueue.filter((e) => e.status === "completed").length,
        },
        {
            value: "cancelled",
            label: "Cancelled",
            count: activeQueue.filter((e) => e.status === "cancelled").length,
        },
    ...
    return (
        <div>
                    ...
                    {rejectedEncounters.map((enc) => (
                        ...
                        >
                            ...
                            <div style={{ flex: 1 }}>
                                ...
                                >
                                    DOCTOR REJECTED CASE
                                </span>
                                <span
                                    style={{ color: "#f1f5f9", marginLeft: 10, fontWeight: 600 }}
                                >
                                    {enc.patient_detail?.name || "Unknown Patient"}
                                </span>
                                <span
            ...
            {activeQueue.filter(
                (e) => e.code_blue_active && !e.code_blue_acknowledged,
            ).length > 0 && (
                    <div
                        style={{
                            display: "flex",
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div style={{ flex: 1 }}>
                                            ...
                                            >
                                                Resuscitation protocol initiated · 🏥 {icuBed} · Requires
                    ...
                    >
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            {enc.code_blue_acknowledged_by_name && (
                                ...
                                >
                                    by {enc.code_blue_acknowledged_by_name}
                                </span>
                            )}
                        </div>
                    </div>
            ...
            <div className="page-header">
                <div>
                    ...
                    <div className="page-subtitle">
                        {activeQueue.length} active · {incomingAmbulances.length} en route —
            ...
            <div style={{ marginBottom: "1.5rem" }}>
                ...
                >
                    ...
                    {incomingAmbulances.length > 0 && (
                        ...
                        >
                            {incomingAmbulances.length} en route
                        </span>
                    )}
                </div>
                {incomingAmbulances.length === 0 ? (
            ...
            {filteredQueue.length === 0 ? (
                <div className="empty-state">
                    ...
                    <p>
                        No{" "}
                        {statusFilter === "all" ? "active" : statusFilter.replace("_", " ")}{" "}
                        encounters
                    </p>
                </div>
            ) : (
                            ...
                            {filteredQueue.map((enc) => (
                                ...
                                >
                                    <td>
                                        ...
                                        >
                                            {enc.patient_detail?.age
                                                ? `${enc.patient_detail.age}y`
                                                : ""}{" "}
                                            {enc.patient_detail?.gender || ""}
                                        </div>
                                        {enc.status === "escalated" && (
                                            <div
                                                style={{
                                    ...
                                    <td>
                                        ...
                                        >
                                            ...
                                            >
                                                {waitLabel(
                                                    enc.created_at,
                                                    ["completed", "cancelled"].includes(enc.status)
                                                        ? enc.updated_at
                                                        : null,
                                                )}
                                            </span>
                                    ...
                                    <td
                                        style={{
                                            color: enc.assigned_doctor_detail
                                                ? "var(--text)"
                                                : "var(--text-muted)",
                                        }}
                                    >
